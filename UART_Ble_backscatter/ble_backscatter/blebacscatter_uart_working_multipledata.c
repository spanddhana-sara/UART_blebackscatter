...
#define UART_DEVICE "/dev/ttyS1" // UART Device path
#define BLE_ADV_REPEAT_COUNT 100
...
int main(int argc, char **argv) {
  ...
  // If beaglebone is not defined, return immediately after printing the BLE payload
   //Open UART
  serial_fd = open(UART_DEVICE,  O_RDWR | O_NOCTTY);
  if (serial_fd < 0){
      perror("Failed to open UART");
      return -1;
  }
  ...
  printf("Waiting for UART input....\n");
   ...
   while (1){
       memset(payload, 0, sizeof(serial_buf));
       bytes_read = read(serial_fd, serial_buf, sizeof(serial_buf) - 1);
       if (bytes_read > 0){
            //payload[bytes_read] = '\0';
	    printf("Received payload: %s\n", serial_buf);

	   generate_ble_adv_payload(&sblePacketInstance, serial_buf, arrMacAddress,39);

           pruMem8 = (uint8_t *) pru0DataMem;
           prumemCnt = 0;
           time = 0;
#ifndef BEAGLEBONE
  ...
  printf("BLE packet generated, length %d\n",sblePacketInstance.u8PayloadLen );
    ...
    for(i=0; i< 100; i++) //i<100 previous
    {
